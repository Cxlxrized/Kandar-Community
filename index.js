...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName("panel")
    .setDescription("📩 Sendet das Kandar Ticket-Panel (Dropdown)"),
  ...
  new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("add")
        .setDescription("Creator hinzufügen (mit Social Links & Rolle)")
    ),
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((o) =>
      o.setName("dauer").setDescription("z. B. 1d 2h 30m").setRequired(true)
    )
    .addIntegerOption((o) =>
      o
        .setName("gewinner")
  ...
  for (const name of Object.values(stats)) {
    if (
      !guild.channels.cache.find(
        (c) => c.parentId === category.id && c.name.startsWith(name)
      )
    ) {
...
/* ===========================
   INTERACTIONS (Befehle, Buttons, Modals)
=========================== */
client.on("interactionCreate", async (i) => {
  try {
    ...
    if (i.isStringSelectMenu() && i.customId === "ticket_select") {
      ...
      // Direkt-Channel Typen
      const map = {
        kandar: {
          title: "✍️ Kandar Bewerbung",
          cat: "✍️ Kandar Bewerbungen",
          desc: "Bitte schreibe deine Bewerbung hier.",
          prefix: "✍️",
        },
        designer: {
          title: "🎨 Designer Bewerbung",
          cat: "🎨 Designer Bewerbungen",
          desc: "Bitte sende dein Portfolio.",
          prefix: "🎨",
        },
        cutter: {
          title: "✂️ Cutter Bewerbung",
          cat: "✂️ Cutter Bewerbungen",
          desc: "Bitte nenne Software & Erfahrung.",
          prefix: "✂️",
        },
        highteam: {
          title: "🛠️ Highteam Ticket",
          cat: "🛠️ Highteam Anliegen",
          desc: "Beschreibe bitte dein Anliegen.",
          prefix: "🛠️",
        },
        support: {
          title: "👥 Support Ticket",
          cat: "👥 Support Anliegen",
          desc: "Beschreibe bitte dein Anliegen.",
          prefix: "👥",
        },
      };
      ...
      let cat = guild.channels.cache.find(
        (c) => c.name === data.cat && c.type === ChannelType.GuildCategory
      );
      if (!cat)
      ...
      const ch = await guild.channels.create({
        name: `${data.prefix}-${i.user.username}`,
    ...
    if (i.isButton() && ["ticket_close", "ticket_transcript", "ticket_delete"].includes(i.customId)) {
      ...
      if (i.customId === "ticket_close") {
        ...
        try {
          // Ersteller bleibt lesen, darf aber nicht mehr schreiben
          await ch.permissionOverwrites.edit(i.user.id, {
            SendMessages: false,
          });
          await ch.send({ embeds: [new EmbedBuilder().setColor(INFO_COLOR).setDescription("🔒 Ticket geschlossen.")] });
        } catch {}
      }

      // Transcript
      if (i.customId === "ticket_transcript") {
        await i.reply({ content: "🧾 Erstelle Transcript…", ephemeral: true });
        try {
          const filePath = await makeTranscript(ch);
          const file = new AttachmentBuilder(filePath);
          await ch.send({ content: "🧾 Transcript erstellt:", files: [file] });
          await i.editReply({ content: "✅ Transcript gesendet!" });
        } catch (e) {
          await i.editReply({ content: "❌ Konnte Transcript nicht erstellen." });
        }
      }

      // Delete
      if (i.customId === "ticket_delete") {
        await i.reply({ content: "🗑️ Ticket wird gelöscht…", ephemeral: true });
        setTimeout(() => ch.delete().catch(() => {}), 1500);
      }
    }

    /* ---- CREATOR ADD (mit Modal & Datei) ---- */
    if (
      i.isChatInputCommand() &&
      i.commandName === "creator" &&
      i.options.getSubcommand() === "add"
    ) {
      const modal = new ModalBuilder()
      ...
      const fields = [
        { id: "title", label: "Titel des Embeds", required: true },
        { id: "creatorId", label: "Discord-ID des Creators", required: true },
        { id: "twitch", label: "Twitch Link (Pflicht)", required: true },
        { id: "youtube", label: "YouTube Link (Optional)", required: false },
        { id: "tiktok", label: "TikTok Link (Optional)", required: false },
        { id: "instagram", label: "Instagram Link (Optional)", required: false },
        { id: "code", label: "Creator Code (Optional)", required: false },
      ];
    ...
    if (i.isModalSubmit() && i.customId === "creatorAddModal") {
      ...
      // Rolle zuweisen (falls vorhanden)
      const member = guild.members.cache.get(creatorId);
      if (member) {
        const role =
          guild.roles.cache.find((r) => r.name.toLowerCase() === "creator") ||
          guild.roles.cache.get(process.env.CREATOR_ROLE_ID);
    ...
    if (i.isChatInputCommand() && i.commandName === "nuke") {
      ...
      try {
        ...
        await ch.send(
          "❌ Fehler beim Löschen (Hinweis: Nachrichten >14 Tage können nicht gelöscht werden)."
        );
      }
    }
    ...
    if (i.isChatInputCommand() && i.commandName === "giveaway") {
      ...
      if (!gewinner || gewinner < 1)
        return i.reply({
          content: "⚠️ Bitte gib eine gültige Gewinneranzahl an!",
          ephemeral: true,
        });
      ...
      if (!dauer || dauer <= 0)
        return i.reply({
          content: "⚠️ Ungültige Dauer (z. B. 1d2h30m)",
          ephemeral: true,
        });
    ...
    // GIVEAWAY JOIN (Teilnehmerzähler im Embed updaten)
    if (i.isButton() && i.customId === "giveaway_join") {
      const giveaways = loadGiveaways();
      const g = giveaways.find((x) => x.messageId === i.message.id);
      if (!g)
        return i.reply({ content: "❌ Giveaway nicht gefunden!", ephemeral: true });
      if (g.beendet)
        return i.reply({
          content: "🚫 Dieses Giveaway ist beendet!",
          ephemeral: true,
        });
      ...
      // Embed-Zeile "Teilnehmer: X" aktualisieren
      const old = i.message.embeds[0];
      const updated = EmbedBuilder.from(old);
      const desc = old?.description || "";
      const newDesc = desc.replace(/👥 \*\*Teilnehmer:\*\* \d+/i, `👥 **Teilnehmer:** ${g.teilnehmer.length}`);
    ...
    if (i.isChatInputCommand() && i.commandName === "reroll") {
      ...
      const winners = Array.from({ length: g.gewinner }, () => {
        const id = g.teilnehmer[Math.floor(Math.random() * g.teilnehmer.length)];
        return `<@${id}>`;
      });
    ...
    if (i.isChatInputCommand() && i.commandName === "streamannounce") {
      ...
      const announceChannelId = process.env.TWITCH_CHANNEL_ID;
      if (!twitchUser || !announceChannelId)
        return i.reply({
          content:
            "⚠️ Bitte `TWITCH_USER` und `TWITCH_CHANNEL_ID` in der `.env` setzen.",
          ephemeral: true,
        });

      const ch = i.guild.channels.cache.get(announceChannelId);
      if (!ch)
        return i.reply({
          content: "❌ Announce-Channel nicht gefunden.",
          ephemeral: true,
        });

      // Twitch-Preview (ohne API, Cache-Bust per timestamp)
      const preview = `https://static-cdn.jtvnw.net/previews-ttv/live_user_${encodeURIComponent(
        twitchUser
      )}-1280x720.jpg?ts=${Date.now()}`;

      const embed = new EmbedBuilder()
        .setColor(BRAND_COLOR)
        .setTitle(`🔴 ${twitchUser} ist jetzt LIVE!`)
        .setDescription(
          `Schau rein und unterstütze den Stream!\n👉 https://twitch.tv/${twitchUser}`
        )
        .setImage(preview)
      ...
      return i.reply({ content: "✅ Stream-Announce gepostet!", ephemeral: true });
    }
  } catch (err) {
...
async function endGiveaway(msgid, interaction = null) {
  ...
  try {
    ...
    if (!g.teilnehmer.length) {
      ...
      if (interaction)
        await interaction.reply({
          content: "❌ Keine Teilnehmer. Giveaway beendet.",
          ephemeral: true,
        });
      return;
    }
    ...
    await msg.edit({ embeds: [embed], components: [] });
    await ch.send(
      `🎉 Glückwunsch ${winners.join(", ")}! Ihr habt **${g.preis}** gewonnen!`
    );
    if (interaction)
    ...
    log.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(`${m} ist beigetreten.`),
      ],
    });
});
    ...
    log.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(`${m.user.tag} hat den Server verlassen.`),
      ],
    });
});
...
client.on("guildUpdate", (oldG, newG) => {
  ...
  if (oldG.name !== newG.name)
    changes.push(`📛 Name: **${oldG.name} → ${newG.name}**`);
